...
Canvas {
    ...
    ParallelAnimation {
        id: shoot
        running: false
        NumberAnimation { id: xAnimation; target: bullet; property: "x"; to: bullet.originPoint.x+Math.cos(bullet.shootAngle* (Math.PI/180))*bullet.range*bullet.scaleFactor; loops: 1; duration: 0.6*bullet.range*bullet.scaleFactor; easing.type: Easing.Linear }
        NumberAnimation { id: yAnimation; target: bullet; property: "y"; to: bullet.originPoint.y-Math.sin(bullet.shootAngle* (Math.PI/180))*bullet.range*bullet.scaleFactor;  loops: 1; duration: 0.6*bullet.range*bullet.scaleFactor; easing.type: Easing.Linear }
        onStopped: {
